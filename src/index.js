...
app.post('/image-diff-by-base64', function (req, res) {
  ...
  if (image1 === undefined || image2 === undefined) {
    message.error =
      'Required two argument image1 and image2, use base64 from of file data'
  ...
  function doneReading () {
    ...
    if (diffpixcount > 0) {
      ...
      diff.pack().pipe(
        fs.createWriteStream(difffile).on('close', function () {
  ...
  function donewriting (fs, fd) {
    ...
    fsr1 = fs
      .createReadStream(image1FilePath)
      .pipe(new PNG())
      .on('parsed', doneReading)
    fsr2 = fs
      .createReadStream(image2FilePath)
      .pipe(new PNG())
      .on('parsed', doneReading)
  }
